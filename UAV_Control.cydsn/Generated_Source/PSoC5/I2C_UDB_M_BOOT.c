...
static uint8 XDATA I2C_UDB_M_slReadBuf[I2C_UDB_M_BTLDR_SIZEOF_READ_BUFFER];

/* I2C read buffer - The host reads responses from it */
static uint8 XDATA I2C_UDB_M_slWriteBuf[I2C_UDB_M_BTLDR_SIZEOF_WRITE_BUFFER];


/*******************************************************************************
* Function Name: I2C_UDB_M_CyBtldrCommStart
********************************************************************************
*
* Summary:
...
*
* Global variables:
*  I2C_UDB_M_slWriteBuf - The global variable used to store a received
*                                command.
*  I2C_UDB_M_slReadBuf -  The global variable used to store a response.
...
void I2C_UDB_M_CyBtldrCommStart(void) CYSMALL 
{
    ...
    I2C_UDB_M_SlaveInitWriteBuf(I2C_UDB_M_slWriteBuf, I2C_UDB_M_BTLDR_SIZEOF_WRITE_BUFFER);
    I2C_UDB_M_SlaveInitReadBuf (I2C_UDB_M_slReadBuf, 0u);
...
/*******************************************************************************
* Function Name: I2C_UDB_M_CyBtldrCommStop
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: I2C_UDB_M_CyBtldrCommReset
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: I2C_UDB_M_CyBtldrCommWrite
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
cystatus I2C_UDB_M_CyBtldrCommWrite(const uint8 pData[], uint16 size, uint16 * count, uint8 timeOut) CYSMALL
...
/*******************************************************************************
* Function Name: I2C_UDB_M_CyBtldrCommRead
********************************************************************************
*
* Summary:
...
*
* Global variables:
*  I2C_UDB_M_slWriteBuf - The global variable used to store a
*                                received command.
*
*******************************************************************************/
         ...
         
{
    ...
    if((NULL != pData) && (size > 0u))
    {
        ...
        while(0u != timeoutMs)  /* Wait for command from host */
        {
            ...
            if(0u != (I2C_UDB_M_slStatus & I2C_UDB_M_SSTAT_WR_CMPLT))
            {
                /* Define how many bytes host has been written */
                byteCount = I2C_UDB_M_slWrBufIndex;
                *count = (uint16) byteCount;

                /* Copy command to pData buffer */
                (void) memcpy((void *) pData, (void *) I2C_UDB_M_slWriteBuf,
                              I2C_UDB_M_MIN_UNT16(byteCount, size));
